...
for epoch in range(1, pretrain_epochs + 1):
    new_learning_rate = 0.001 - (0.001 - 0.0005) * (epoch - 1) / (pretrain_epochs - 1)
    ...
    for batch_n in tqdm(range(1, batches_per_epoch+1)):
        ...
        if batch_n % 200 == 0:
            ...
            save_examples(examples_folder, articles, gt_summaries, summaries, epoch, batch_n, 'val', stage='pretrain')
            save_scores(metrics_folder, scores, 'pretrain_val')

            mean_epoch_loss = np.mean(val_losses)
            save_loss(metrics_folder, mean_epoch_loss, 'pretrain_val')

        if batch_n % 600 == 0:
            save_model(model, model_checkpoints, epoch, batch_n, stage='pretrain')

    save_model(model, model_checkpoints, epoch, 'last', stage='pretrain')


ce_loss.set_alpha(0.01)
rl_loss.set_alpha(0.99)
        ...
        if check_shapes(batch):
            ...
            if batch_n % 200 == 0:
                ...
                save_examples(examples_folder, articles, gt_summaries, summaries, epoch, batch_n, 'train',
                              stage='RL', in_graph_decodings=in_graph_decodings)
                ...
                examples_oovs = [val_oovs[x] for x in val_inds]
                scores, summaries, time_step_masks = env.get_rewards(gt_summaries, val_sums, examples_oovs)
                save_examples(examples_folder, articles, gt_summaries, summaries, epoch, batch_n, 'val', stage='RL')
                save_scores(metrics_folder, scores, 'rl_val')

                mean_epoch_loss = np.mean(val_losses)
                save_loss(metrics_folder, mean_epoch_loss, 'rl_val')
